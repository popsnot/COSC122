...
class ChainingHashTable():
    ...
    ***** DON'T worry you will fix this later:)
    ...
    def _hash(self, item):
        ...
        pre_modulo_hash = nice_hash(item)
        return pre_modulo_hash % self.n_slots
    ...
    def __contains__(self, item):
        ...
        slot_chain = self._data[slot]
        for value in slot_chain:
            if value == item:
                return True
        return False
...
class LinearHashTable():
    ...
    >>> hash_table.store('NIGGER')
    >>> hash_table.store('cuwiuefbm')
    >>> hash_table.store('cunt')
    >>> print(hash_table)
    LinearHashTable:
    slot        0 = David
    slot        1 = Darlene
    ...
    def store(self, item):
        ...
        item_hash = self._hash(item) % self.n_slots
        probe_counter = 0
        if self._data[item_hash] != None:
            while self._data[item_hash] != None and probe_counter <= self.n_slots:
                item_hash = (item_hash + 1) % self.n_slots
            self._data[item_hash] = item
        else:
            self._data[item_hash] = item
        self.n_items += 1
...
class QuadraticHashTable():
    ...
    def store(self, item):
        ...
        og_hash = self._hash(item)
        item_hash = self._hash(item)
        i = 0
        if self._data[item_hash] == None:
            self._data[item_hash] = item
        else:
            while self._data[item_hash] != None:
                item_hash = (og_hash+(i**2)) % self.n_slots
                i += 1
            self._data[item_hash] = item
        self.n_items += 1
